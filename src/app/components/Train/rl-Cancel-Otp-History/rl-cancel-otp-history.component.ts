...
import {RlOtpDetailsComponent} from "../rl-Otp-Details/rl-otp-details.component"
@Component({
  selector: 'app-rl-cancel-otp-history',
  templateUrl: './rl-cancel-otp-history.component.html',
  styleUrls: ['./rl-cancel-otp-history.component.scss'],
})
export class RlCancelOtpHistoryComponent implements OnInit {
  ...
  RL_CANCEL_OTP_DETAILS(d) {
    this.present()
    let obj = {
      "P_TYPE": "CC",
      "R_TYPE": "RAIL",
      "R_NAME": "RL_CANCEL_OTP_DETAILS",
      "R_DATA": {
        "RAID": d.RID,
        "BOOKING_ID": d.BOOKING_ID,
        "CANCELATION_ID": d.CANCELATION_ID
      },
      "AID": this.login_Details.AID,
      "MODULE": this.login_Details.MODULE,
      "IP": this.login_Details.IP,
    ...
    this.tService.postTestData(obj).subscribe(result => {
      if (result.response !== null) {
        this.dismiss()
        this.presentModal1()
        localStorage.setItem("otp_Details", result.response)
      }
      else {
        alert("No Data Found")
        this.dismiss()
  ...
  async presentModal1() {
    const modal = await this.modalController.create({
      component: RlOtpDetailsComponent,
      cssClass: 'popover_setting3',
